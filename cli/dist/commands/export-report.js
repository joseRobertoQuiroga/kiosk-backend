...
async function exportReportCommand(outputPath) {
    if (!(0, config_1.isAuthenticated)()) {
        logger_1.logger.error('Debes hacer login primero');
        process.exit(1);
    }
    logger_1.logger.header('📊 EXPORTAR REPORTE DE LICENCIAS');
    const api = new api_1.ApiClient();
    const spinner = (0, ora_1.default)('Generando reporte...').start();
    try {
        // 1. Obtener datos
        ...
        if (licenses.length === 0) {
            ...
            licenses.forEach((license, index) => {
                ...
                report += `    Expira: ${license.expiry_date
        ...
        if (clients.length === 0) {
            ...
            clients.forEach((client, index) => {
                ...
                if (client.tax_id)
                    report += `    RUC/NIT: ${client.tax_id}\n`;
                if (client.contact_email)
                    report += `    Email: ${client.contact_email}\n`;
                if (client.city)
                    report += `    Ciudad: ${client.city}\n`;
        ...
        console.log('');
        logger_1.logger.success(`Archivo guardado en: ${filepath}`);
        logger_1.logger.info(`Total de licencias: ${licenses.length}`);
        logger_1.logger.info(`Total de clientes: ${clients.length}`);
        logger_1.logger.info(`Total de dispositivos: ${devices.length}`);
        console.log('');
    }
    catch (error) {
